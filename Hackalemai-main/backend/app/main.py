...
def health() -> dict[str, Any]:
    # Never expose the key itself; the UI and setup checks only need its presence.
    return {"ok": True, "demo_mode": DEMO_MODE, "products": len(PRODUCTS), "ai_enabled": bool(os.getenv("OPENAI_API_KEY", "").strip())}
...
def chat(payload: ChatRequest) -> dict[str, Any]:
    ...
    if state.get("pending") and (yes_intent(payload.message) or no_intent(payload.message) or extract_quantity(payload.message) is not None):
        ...
        model_result = None
        # Keep cart quantities deterministic, but let the model handle all other
        # questions when configured. Previously most real catalog questions were
        # routed around the model solely because they contained a known keyword.
        is_cart_quantity = extract_quantity(payload.message) is not None and bool(re.search(r"(нужн|надо|хочу|добав|штук|шт\b|количеств)", payload.message.lower().replace("ё", "е")))
        if not is_cart_quantity:
